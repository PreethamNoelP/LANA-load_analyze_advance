...
| 🔬 **Investigate — the model proposes, the engine decides** | Pick a metric and LANA's local model proposes several candidate drivers from the schema; each is then tested for real (Kruskal-Wallis or Pearson correlation) against your actual rows, and the whole batch is corrected for testing multiple hypotheses at once with the same Benjamini-Hochberg routine the correlation scan uses. Only what survives correction is reported as supported — a hypothesis with a raw p < 0.05 that fails correction is shown as unsupported, not hidden. The model never judges whether a hypothesis holds; it proposes and narrates, the statistics decide. `POST /investigate`. |
...
| 🚨 **Crash visibility, not a black box** | A route that genuinely crashes is caught, logged with a full traceback and the request id, counted separately from an ordinary error response (`lana_unhandled_exceptions_total`), and never leaks a stack trace to the caller. No new dependency — example Prometheus/Alertmanager rules in [`docs/alerting.md`](docs/alerting.md) wire it into real email/Slack/PagerDuty alerting using tooling LANA already speaks the exposition format for. |
| 💽 **Automated local backups** | `python -m scripts.backup run` snapshots accounts, persisted sessions and the audit trail into a rotating local archive with retention, separate from `LANA_DATA_DIR` so losing that directory doesn't take the backups too. See [`docs/backup-and-restore.md`](docs/backup-and-restore.md) for scheduling (cron, Task Scheduler, Docker) and the restore procedure. |
...
│  POST /investigate     →  model proposes drivers of a metric, real    │
│                           stats tests decide, FDR-corrected as a batch│
...
A structured fact ledger prepended to every prompt reduces hallucination, but it doesn't prove an answer is correct on its own. Built a small offline eval harness (`eval/`) that generates seeded synthetic datasets, resolves ground truth from LANA's own statistics functions, and runs 40 labeled questions against a real local model under two conditions (grounded vs. a naive baseline). First run: grounded answers scored *worse* than the baseline on trivial questions — the model was pattern-matching a single refusal example in the system prompt almost verbatim, regardless of whether the fact was actually present. Fixed by rewording the prompt's final instruction to model search-then-answer instead of licensing refusal as the default (full account in `docs/engineering-changelog.md`). Full-suite result after the fix: 82.5% correct vs. 52.5% for the baseline — and the eval run itself surfaced a genuine blind spot: a number correctly matched to the wrong column or label, which the validator was reporting as *verified*. That is now caught when the answer names the wrong column or category literally, by grouping facts into families and checking whether a number is quoted under a sibling's name. What remains uncaught is a mislabelling phrased as a paraphrase that never names either side — documented rather than hidden, both in `docs/provenance.md` and in a collapsible panel in the Ask AI UI, and pinned by `tests/test_adversarial_suite.py` so the boundary cannot drift silently in either direction.
